...
<article class="side_story">
    ...
    <div class="meta" style="font-size: 11px;">
        <?php
                ...
                ?>
        <a href="javascript:;"
            onclick="shareData('{{ $title }}','{{ (isset($img_d) ? $img_d: "") }}','{{ url('/'.$bc_2->year.'/'.$bc_2->month.'/'.$bc_2->slug) }}.html')"
            class="share_link">
            <img src="{{ url('/images/icon_share.png') }}" class="lazy" style="padding: 1px;"
                alt="">
        </a>
        <div class="post_age"> by Linda Ikeji at
            {{ \Carbon\Carbon::parse($bc_2->publish_date)->format('d/m/Y') }}</div>
