                        ...
                        <div class="col-md-12">
                            <article class="big_story" style="word-wrap: break-word;">
                                ...
                                <summary class="description"
                                    style="font-size: 16px !important;line-height: 21px;">
...
//
                                ...
                                <div class="meta">
                                    <?php
                                        ...
                                        ?>
                                    <a href="javascript:;"
                                        onclick="shareData('{{ $title }}','{{ (isset($img_d) ? $img_d: "") }}','{{ website_url('/p/'.$b_c->year.'/'.$b_c->month.'/'.$b_c->slug) }}.html')"
                                        class="share_link">
                                        <img src="{{ website_url('/images/icon_share.png') }}"
                                            alt=""></a>
                                    <div class="post_age"> by Linda Ikeji at
                                        {{ \Carbon\Carbon::parse($b_c->publish_date)->format('d/m/Y g:i A') }}
                                    </div>
                                    <div class="divider">|</div>
                                    <div class="comments"><a style="color: #000;"
                                            href="{{ website_url('/'.$b_c->year.'/'.$b_c->month.'/'.$b_c->slug) }}.html#comments">{{ $b_c->approved_comments_count->count() }}
                                            Comments</a></div>
                                </div>
                                <div class="meta">
                                    <h5 style="color: #000;">Share this Story</h5>
                                    <div id="share"></div>
                                </div>
                        ...
                        <div class="text-center">
                            ...
                            <ins class="adsbygoogle" style="display:block"
                                data-ad-client="ca-pub-2427099653703778" data-ad-slot="8308195748"
                                data-ad-format="auto"></ins>
            ...
            (adsbygoogle = window.adsbygoogle || []).push({});
                    ...
                    <div class="more_stories flexslider" style="margin-top: 80px;">
                        ...
                        <ul class="slides" style="display:none;">
                            ...
                            <li class="car_item" style="width: 180px;@if($i > 4)
                                ...
                                @if(auth()->check())
                                <a
                                    href="{{ url('/p/'.$b_c_v->year.'/'.$b_c_v->month.'/'.$b_c_v->slug) }}.html">

                                    @else
                                    <a
                                        href="{{ url('/'.$b_c_v->year.'/'.$b_c_v->month.'/'.$b_c_v->slug) }}.html">
                    ...
                    <div class="comments_area">
                        <h3 id="comments">Comments
                            <span>({{ $b_c->approved_comments_count->count() }})</span></h3>
                        <!-- comment div start -->
                        ...
                        <div class="comment">
                            <a href="#" class="pic" style="overflow:hidden">
                                <img src="
                                            @if(isset($comment->img_url) && !empty($comment->img_url) && $comment->img_url != "
                                    undefined") {{ $comment->img_url }} @else
                                    {{ url('img/default_avatar.png') }} @endif" alt="">
                            </a>
                            <div class="comment_holder">
                                <div class="comment_top">
                                    <a href="#" class="profile_name"
                                        style="font-size: 14px;">{{ $comment->name }}</a> about
                                    {{ \Carbon\Carbon::parse($comment->created_at)->diffForHumans() }}
                                </div>
                                <p style="font-size: 16px; margin-top: 6px;">
                                    {{ utf8_decode($comment->comments) }}
                                </p>
                                <a href="javascript:;" onclick="likeComment('{{ $comment->id }}')"
                                    class="btn btn-danger btn-xs"
                                    style="font-size:10px;line-height: 16px;background: #8d112b;padding: 0px 4px;">Like
                                    this! <span
                                        class="count_{{ $comment->id }}">@if($comment->likes){{ $comment->likes }}@else
                                        0
                                        @endif</span></a>

                                <a href="javascript:;" onclick="dlikeComment('{{ $comment->id }}')"
                                    class="btn btn-danger btn-xs"
                                    style="font-size:10px;line-height: 16px;background: #8d112b;padding: 0px 4px;">Dislike
                                    this! <span
                                        class="dcount_{{ $comment->id }}">@if($comment->dislikes){{ $comment->dislikes }}@else
                                        0
                                        @endif</span></a>

                                <a href="javascript:;" onclick="replyComment('{{ $comment->id }}')"
                                    class="btn btn-danger btn-xs"
                                    style="font-size:10px;line-height: 16px;background: #8d112b;padding: 0px 4px;">
                                    Reply</a>

                                @if(auth()->check())
                                <a href="javascript:;"
                                    onclick="declineC('{{ url('/decline/comment/'.encrypt_decrypt('encrypt',$comment->id)) }}')"
                                ...
                                @if($advert_count % 4 == 0 && $advert_count < 17)
                                    ...
                                    <div class="comment reply">
                                        <a href="#" class="pic" style="overflow:hidden">
                                            <img src="@if(isset($child['img_url']) && !empty($child['img_url']) && $child['img_url'] != "
                                                undefined") {{ $child['img_url'] }} @else
                                                {{ url('img/default_avatar.png') }} @endif" alt="">
                                        </a>
                                        <div class="comment_holder">
                                            <div class="comment_top">
                                                <a href="#" class="profile_name"
                                                    style="font-size: 14px;">{{ $child['name'] }}</a>
                                                about
                                            ...
                                            <a href="javascript:;"
                                                ...
                                                style="font-size:10px;line-height: 16px;background: #8d112b;padding: 0px 4px;">Like
                                                this! <span
                                                    class="count_{{ $child['id'] }}">@if($child['likes']){{ $child['likes'] }}@else
                                                    0
                                                    @endif</span></a>
                                            <a href="javascript:;"
                                                onclick="dlikeComment('{{ $child['id'] }}')"
                                                class="btn btn-danger btn-xs"
                                                style="font-size:10px;line-height: 16px;background: #8d112b;padding: 0px 4px;">Dislike
                                                this! <span
                                                    class="dcount_{{ $child['id'] }}">@if($child['dislikes']){{ $child['dislikes'] }}@else
                                                    0
                                                    @endif</span></a>
                                            <a href="javascript:;" class="btn btn-danger btn-xs"
                                                style="font-size:10px;line-height: 16px;background: #8d112b;padding: 0px 4px;"
                                                onclick="replyComment('{{ $child['id'] }}')">Reply</a>
                                            @if(auth()->check())
                                            <a href="javascript:;"
                                                onclick="declineC('{{ url('/decline/comment/'.encrypt_decrypt('encrypt',$child['id'])) }}')"
                                                class="btn btn-xs btn-danger pull-right">Delete</a>
                                            @endif
                                        </div>
                                    </div>
                        ...
                        <div class="comment_field_area" id="postcomment">
                            ...
                            <div style="display:none" class="loading" style="margin:auto;"><img
                                    style="height: 50px;" src="{{ url('/img/loading.gif') }}"
                                    alt="" /></div>
                            <div class="success alert alert-success" style="display:none">Your post
                                has been submitted and will be visible after the blog owner approves
                            </div>

                            <input type="hidden" id="anono" class="form-control">
                            <div class="clearfix"></div>
                            <div class="ano">
                                <input type="text" id="full_name_c" placeholder="Fullname"
                                    class=" form-control"><br />
                                <input type="text" id="email_c" placeholder="Email"
                                    class=" form-control"><br />
                            </div>
                            <input type="hidden" id="parent_id" class="form-control">

                            <textarea id="comment_c" placeholder="Enter your comment"
                                class="form-control"></textarea><br />
                            <div class="pull-left" style="font-size: 14px;line-height: 50px">
                                <input type="checkbox" id="ano" name="ano"
                                    onclick="changeData()"><label for="ano"
                                    style="margin-left: 5px; font-size: 16px;">Anonymous</label><br />
                            </div>

                            <button class="comment_btn pull-right" onclick="submit_comment()">Submit
                                Comment</button>
                            <div class="clearfix"></div>
                        </div>
